  ...
  class Usuario extends Conexion
  {
    public function login($user, $clave)
    {
      ...
      */
      ...
      $consulta = 'select id_user, name, auth_id from users where email="'.$user.'" and password= MD5("'.$clave.'")';
      /*
    ...
    public function registroUsuario($name, $email, $clave,$dir,$tel)
    {
      ...
      $clave = parent::filtrar($clave);
      $dir=  parent::filtrar($dir);
      $tel=  parent::filtrar($tel);
      // validar que el correo no exito
      $verificarCorreo = parent::verificarRegistros('select id_user from users where email="'.$email.'" ');
      
      if($verificarCorreo > 0){
        echo 'error_3';
      }else{
        parent::query('insert into users(id_user,name,email,password,direccion,telefono,auth_id) values("null","'.$name.'","'.$email.'"'
                    . ',MD5("'.$clave.'"),"'.$dir.'","'.$tel.'",3)');
